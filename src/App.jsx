...
//ABOUT COMPONENT
const About = ({Projects, handleProject}) => {
  ...
  return (
    <div className='font-mono flex flex-col gap-8 pt-50 '>
      ...
      <button className="border-2 border-green-600 text-green-600 w-60 p-5 mt-5 hover:border-green-400 hover:text-green-400 transition duration-50 ease-in-out hover:cursor-pointer" onClick={()=>{Projects(); handleProject}}>Projects</button>
    </div>
  )
}
...
const cardList = [
  ...
  {title: 'Card2', src: 'https://via.placeholder.com/150', alt: 'Gmail image'},
];
...
const Contact = () => {
  ...
  return (
    <div className='text-white border-none'>
      <ul>
        <li><Card src={cardList[0].src} alt={cardList[0].alt}/></li>
        <li><Card src={cardList[1].src} alt={cardList[1].alt}/></li>
        <li><Card src={cardList[2].src} alt={cardList[2].alt}/></li>
      </ul>
    </div>
  )
}
  ...
  return (
    <div className='font-mono flex flex-col bg-[#09182E] fixed h-full w-full'>
      ...
      <div className='flex justify-center'>
        {activeSection === 'about' && <About Projects={activateProjects} />}
